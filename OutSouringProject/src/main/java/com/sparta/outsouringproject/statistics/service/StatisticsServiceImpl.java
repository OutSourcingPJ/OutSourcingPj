...
public class StatisticsServiceImpl implements StatisticsService {
    ...
    public Long getMonthlySalesAmount(Long storeId) {
        LocalDateTime startOfMonth = LocalDate.now().withDayOfMonth(1).atStartOfDay();
        LocalDateTime endOfMonth = startOfMonth.plusMonths(1);
        return orderHistoryRepository.findMonthlySalesAmountByStoreId(storeId, startOfMonth, endOfMonth);
    }
    ...
    public Long getMonthlyOrderCount(Long storeId) {
        LocalDateTime startOfMonth = LocalDate.now().withDayOfMonth(1).atStartOfDay();
        LocalDateTime endOfMonth = startOfMonth.plusMonths(1);
        return orderHistoryRepository.findMonthlyOrderCountByStoreId(storeId, startOfMonth, endOfMonth);
    }
