...
public class OrderLogAop {
    ...
    public void logOrderCreation(JoinPoint joinPoint, Object response) {
        ...
        log.info("가게 ID: {} / 주문 ID: {} / 요청시각: {}", cur, storeId, orderId);
    }

    @AfterReturning("execution(* com.sparta.outsouringproject.order.service.OrderServiceImpl.acceptOrder(..)) || " +
        "execution(* com.sparta.outsouringproject.order.service.OrderServiceImpl.startDelivery(..)) || " +
        "execution(* com.sparta.outsouringproject.order.service.OrderServiceImpl.completeOrder(..))")
